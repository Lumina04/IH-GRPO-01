...
def compute_hierarchy_policy_loss(tool_log_probs, tool_logits, non_tool_mask, c_loss_mask, old_log_prob, log_prob, advantages, eos_mask, cliprange_low, cliprange_high, clip_ratio_c=3.0):#, responses=None):
    """Adapted from https://github.com/huggingface/trl/blob/main/trl/trainer/ppo_trainer.py#L1122
    ...
    tool_log_probs_clamped = torch.clamp(tool_log_probs, max=-1e-7)  # ensure exp(tool_log_probs) < 1
    tool_prob_clamped = torch.clamp(torch.exp(tool_log_probs_clamped), min=1e-7, max=1.0 - 1e-7)

    log_sum_exp = tool_logits - tool_log_probs_clamped  # [N]
    logZ = log_sum_exp + torch.log1p(-tool_prob_clamped)  # [N]
    gamma = (1 - tool_prob_clamped).detach()  # [N]
    c_item = (gamma - non_tool_mask) * logZ  # [N]
    c_loss = c_item * pg_losses.detach() * c_loss_mask

    c_loss = verl_F.masked_mean(c_loss, eos_mask)

    return pg_loss, pg_clipfrac, ppo_kl, pg_clipfrac_lower, c_loss
