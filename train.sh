...
VALID_DATASET=("deepscaler/amc23" "deepscaler/aime24" "deepscaler/aime25")
...
generate_suffix() {
  ...
  while [[ "$#" -gt 0 ]]; do
    case $1 in
      ...
      --val_before_train) suffix+="_val_before_train$2"; shift 2;;
      ...
      --add_c_loss) suffix+="_addcloss$2"; shift 2 ;;
...
while [[ "$#" -gt 0 ]]; do
  ...
  case "$1" in
    ...
    --add_c_loss) ADD_C_LOSS="$2"; shift 2 ;;
...
sleep 3
PYTHONUNBUFFERED=1 python3 -m recipe.simpletir.main_simpletir \
    ...
    actor_rollout_ref.actor.add_c_loss=$ADD_C_LOSS \
