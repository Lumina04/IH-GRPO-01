...
def compute_data_metrics(batch, use_critic=True):
    ...
    if "delay_call_time" in batch.meta_info:
        metrics["env/delay_call_time"] = float(
            np.array(batch.meta_info["delay_call_time"], dtype=np.int16).mean()
        )
        delay_call_time = np.array(batch.meta_info["delay_call_time"], dtype=np.int16).sum()
        tool_call_time = np.array(batch.meta_info["turns_stats"], dtype=np.int16).sum()-np.array(batch.meta_info["turns_stats"], dtype=np.int16).shape[0]+delay_call_time
        metrics["env/delay_call_rate"] = float(delay_call_time/tool_call_time)
...
class RaySimpleTIRTrainer(RayPPOTrainer):
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            if pad_size != 0:
                num_turns = test_output_gen_batch.meta_info["turns_stats"][:-pad_size]
                delay_call_time = test_output_gen_batch.meta_info["delay_call_time"][:-pad_size]
            else:
                num_turns = test_output_gen_batch.meta_info["turns_stats"]
                delay_call_time = test_output_gen_batch.meta_info["delay_call_time"]
        ...
        data_source_reward = {}
        data_source_tooluse = {}
        data_source_delaytime = {}
        for i in range(reward_tensor.shape[0]):
            data_source = data_sources[i]
            if data_source not in data_source_reward:
                data_source_reward[data_source] = []
                data_source_tooluse[data_source] = []
                data_source_delaytime[data_source] = []
            data_source_reward[data_source].append(reward_tensor[i].item())
            data_source_tooluse[data_source].append(num_turns[i])
            data_source_delaytime[data_source].append(delay_call_time[i])
        ...
        for data_source, rewards in data_source_reward.items():
            ...
            metric_dict[f"val/mean_tooluse_times/{data_source}"] = np.mean(data_source_tooluse[data_source])
            metric_dict[f"val/delaycall_rate/{data_source}"] = np.sum(data_source_delaytime[data_source])/(np.sum(data_source_tooluse[data_source])-len(data_source_tooluse[data_source])+np.sum(data_source_delaytime[data_source]))
            print(
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with _timer("step", timing_raw):
                    ...
                    if self.config.actor_rollout_ref.actor.add_c_loss:
                        batch = self.calculate_c_loss_mask(batch)
                    ...
                    with _timer("adv", timing_raw):
                        ...
                        if rollout_data_dir and self.global_steps%5 == 1:
                            self._log_rollout_data(batch, rollout_data_dir)
                    ...
                    if self.global_steps >= 200 and self.config.trainer.save_freq > 0 and (
                        is_last_step
    ...
    def calculate_c_loss_mask(self, batch):
        """
        Set c_loss_mask=1 for:
        1. The last token of text from ``` to first non-whitespace char after code block
        2. All tokens with id equal to tool_id
        """
        batch.batch["c_loss_mask"] = batch.batch["response_mask"]
        return batch
        
    def save_response(self, batch, save_path="./model_response.parquet"):
        import pandas as pd
        import pyarrow as pa
        import pyarrow.parquet as pq
        data = []
        for i in range(len(batch)):
            prompt = self.tokenizer.decode(batch.batch["prompts"][0], skip_special_tokens=True)
            decoded_input = self.tokenizer.decode(batch.batch["input_ids"][i], skip_special_tokens=True)
            turns_stats = batch.meta_info["turns_stats"][i]
